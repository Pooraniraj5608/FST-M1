...
SELECT grade,count(customer_id) from customers WHERE grade >(SELECT AVG(grade) from customers where city = 'New York');

SELECT grade,count(customer_id) from customers GROUP BY grade WHERE grade >(SELECT AVG(grade) from customers where city = 'New York');
...
WHERE 1<(SELECT COUNT(*) FROM orders b WHERE a.customer_id = b.customer_id) 
UNION 
SELECT customer_id, customer_name FROM salesman a 
WHERE 1<(SELECT COUNT(*) FROM orders b WHERE a.salesman_id = b.salesman_id) 
ORDER BY customer_name;
...
elect order_no, purchase_amount from orders order by purchase_amount DESC;
